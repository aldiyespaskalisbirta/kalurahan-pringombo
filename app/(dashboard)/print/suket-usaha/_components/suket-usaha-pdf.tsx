...
function SuketUsahaPDF({ data }: Props) {
  ...
  const downloadPDF = () => {
    ...
    generatePDF(() => document.getElementById("container"), {
      method: "save",
      filename: `surat-keterangan-usaha-${decodeData(data?.individu.nik)}.pdf`,
  ...
  const onDelete = async () => {
    try {
      ...
      await axios.delete(`/api/print/suket-usaha/${data?.surat?.no_surat}`);
      toast.success("Berhasil dihapus");
      router.push("/print/suket-usaha");
      router.refresh();
    } catch {
        ...
        {data?.surat && (
          <div className="w-full flex justify-between items-center">
            ...
            <div className="flex gap-x-4">
              <Link
                href="/print/suket-usaha/[nik]/[no_surat]/edit"
                as={`/print/suket-usaha/${data.individu.nik}/${data?.surat.no_surat}/edit`}
              >
