...
export const EditNamaInstansi = ({ initialData, nik }: Props) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      const formatString = values.nama_instansi.toUpperCase();
      await axios.patch(`/api/print/sktm/${initialData.no_surat}`, {
        nama_instansi: formatString,
      });
    ...
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      ...
      {!isEditing && (
        <div
          className={cn(
            "text-sm mt-2",
            !initialData.nama_instansi && "text-slate-500 italic"
          )}
        >
          {!initialData.nama_instansi
            ? "Tidak diketahui"
            : initialData.nama_instansi}
        </div>
      )}
