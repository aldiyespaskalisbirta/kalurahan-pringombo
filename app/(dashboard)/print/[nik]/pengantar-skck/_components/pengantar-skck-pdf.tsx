...
function PengantarSkckPDF({ nik, data }: Props) {
  ...
  const onDelete = async () => {
    try {
      await axios.delete(
        `/api/penduduk/${nik}/pengantar-skck/${data.surat?.no_surat}`
      );
      toast.success("Berhasil dihapus");
      router.refresh();
      router.push(`/print/${nik}/pengantar-skck`);
    } catch {
        ...
        {data?.surat && (
          <div className="w-full flex justify-between items-center">
            ...
            <div className="flex gap-x-4">
              <Link
                href="/print/[nik]/pengantar-skck/[no_surat]"
                as={`/print/${nik}/pengantar-skck/${data.surat.no_surat}/edit`}
              >
